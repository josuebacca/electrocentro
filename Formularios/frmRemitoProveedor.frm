...
Private Sub CmdGrabar_Click()
    ...
    If rec.EOF = True Then 'NUEVO REMITO
        ...
        sql = sql & "TCO_CODIGO,RPR_TOTAL,RPR_SUBTOTAL,RPR_SALDO)"
        sql = sql & " VALUES ("
       ...
       ' If chkRemitoSinFactura.Value = Checked Then
            For I = 1 To grdGrilla.Rows - 1
                If grdGrilla.TextMatrix(I, 0) <> "" Then
                        ...
                        'sql = sql & " WHERE STK_CODIGO=" & cboStock.ItemData(cboStock.ListIndex)
                        sql = sql & " WHERE "
                        sql = sql & " PTO_CODIGO=" & XN(grdGrilla.TextMatrix(I, 0))
                        DBConn.Execute sql
                End If
            Next
       ' End If
    ...
    Else
        ...
        'If MsgBox("Confirma modificar el Remito Nro.: " & Trim(txtNroSucursal.Text) & "-" & Trim(txtNroNotaPedido.Text) & " ", vbQuestion + vbYesNo, TIT_MSGBOX) = vbYes Then
            ...
            For I = 1 To grdGrilla.Rows - 1
                ...
                End If
            Next
            DBConn.Execute QuitoCtaCteProveedores(cmdBuscarCliente.Tag _
                                              , TxtCodigoCli, 21 _
                                        , txtNroSucursal, txtNroRemito)
...
Private Sub Form_Load()
    ...
    grdGrilla.FormatString = "Código|Descripción|Cantidad|Precio|Importe|Rubro|Linea|Orden|Stock"
    grdGrilla.ColWidth(0) = 1400 'CODIGO
    grdGrilla.ColWidth(1) = 5400 'DESCRIPCION
    ...
    grdGrilla.Rows = 1
    For I = 2 To 14
        grdGrilla.AddItem "" & Chr(9) & "" & Chr(9) & "" & Chr(9) & "" & Chr(9) & "" & Chr(9) & "" & Chr(9) & "" & Chr(9) & (I - 1) & Chr(9) & ""
    Next
...
'        CmdNuevo_Click
        ...
        Case 1 'BUSCA REMITOS
            ...
            FechaNotaPedido.Text = GrdModulos.TextMatrix(GrdModulos.RowSel, 8)
        ...
        '----BUSCO DETALLE DEL REMITO------------------
            ...
            If Rec1.EOF = False Then
                ...
                Do While Rec1.EOF = False
                    ...
                    grdGrilla.TextMatrix(I, 7) = Rec1!DRPR_NROITEM
                    grdGrilla.TextMatrix(I, 8) = IIf(IsNull(Rec1!DRPR_CANTIDAD), 0, Rec1!DRPR_CANTIDAD)
                    I = I + 1
                    Rec1.MoveNext
                Loop
            End If
...
Private Sub TxtCodigoCli_LostFocus()
    ...
    If TxtCodigoCli.Text <> "" Then
        ...
        sql = sql & "L.LOC_DESCRI,P.PRO_DESCRI,L.LOC_CODPOS,TP.TPR_CODIGO"
        sql = sql & " FROM PROVEEDOR C, LOCALIDAD L, PROVINCIA P,TIPO_PROVEEDOR TP"
        sql = sql & " WHERE "
        sql = sql & "C.LOC_CODIGO = L.LOC_CODIGO AND "
        sql = sql & "C.PRO_CODIGO = P.PRO_CODIGO AND "
        sql = sql & "L.PRO_CODIGO = P.PRO_CODIGO AND "
        sql = sql & "C.TPR_CODIGO = TP.TPR_CODIGO AND "
...
Private Sub txtNroNotaPedido_LostFocus()
    ...
    If txtNroNotaPedido.Text <> "" Then
        ...
        '-----BUSCO LOS DATOS DEL DETALLE DE LA Orden de Compra---------
            sql = "SELECT DNP.*,P.PTO_DESCRI, P.PTO_PRECIOC, R.RUB_DESCRI, L.LNA_DESCRI"
            sql = sql & " FROM DETALLE_ORDEN_COMPRA DNP, PRODUCTO P, RUBROS R, LINEAS L" ', LISTA_PRECIO D
            ...
            If Rec1.EOF = False Then
                ...
                Do While Rec1.EOF = False
                    ...
                    grdGrilla.TextMatrix(I, 3) = Valido_Importe(Rec1!PTO_PRECIOC)
