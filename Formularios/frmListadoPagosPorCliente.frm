...
'            Rep.SelectionFormula = "{PAGOS_CLIENTES_V.RCL_TCO_CODIGO}=" & cboFactura.ItemData(cboFactura.ListIndex)
            Rep.SelectionFormula = Rep.SelectionFormula & " AND {PAGOS_CLIENTES_V.RCL_NUMERO}=" & XN(txtNroFactura)
        Else
'            Rep.SelectionFormula = Rep.SelectionFormula & " AND {PAGOS_CLIENTES_V.RCL_TCO_CODIGO}=" & cboFactura.ItemData(cboFactura.ListIndex)
            Rep.SelectionFormula = Rep.SelectionFormula & " AND {PAGOS_CLIENTES_V.RCL_NUMERO}=" & XN(txtNroFactura)
        End If
    End If
    ...
    If FechaDesde.Text <> "" Then
        If Rep.SelectionFormula = "" Then
            Rep.SelectionFormula = " {PAGOS_CLIENTES_V.REC_FECHA}>= DATE (" & Mid(FechaDesde.Text, 7, 4) & "," & Mid(FechaDesde.Text, 4, 2) & "," & Mid(FechaDesde.Text, 1, 2) & ")"
        Else
            Rep.SelectionFormula = Rep.SelectionFormula & " AND {PAGOS_CLIENTES_V.REC_FECHA}>= DATE (" & Mid(FechaDesde.Text, 7, 4) & "," & Mid(FechaDesde.Text, 4, 2) & "," & Mid(FechaDesde.Text, 1, 2) & ")"
        End If
    End If
    
    If FechaHasta.Text <> "" Then
        If Rep.SelectionFormula = "" Then
            Rep.SelectionFormula = " {PAGOS_CLIENTES_V.REC_FECHA}<= DATE (" & Mid(FechaHasta.Text, 7, 4) & "," & Mid(FechaHasta.Text, 4, 2) & "," & Mid(FechaHasta.Text, 1, 2) & ")"
                           
        Else
            Rep.SelectionFormula = Rep.SelectionFormula & " AND {PAGOS_CLIENTES_V.REC_FECHA}<= DATE (" & Mid(FechaHasta.Text, 7, 4) & "," & Mid(FechaHasta.Text, 4, 2) & "," & Mid(FechaHasta.Text, 1, 2) & ")"
        End If
    End If
...
'        sql = sql & " WHERE RCL_TCO_CODIGO=" & cboFactura.ItemData(cboFactura.ListIndex)
        sql = sql & " WHERE RCL_NUMERO=" & XN(txtNroFactura)
        If txtCliente.Text <> "" Then sql = sql & " AND CLI_CODIGO=" & XN(txtCliente)
        rec.Open sql, DBConn, adOpenStatic, adLockOptimistic
        If rec.EOF = False Then
            txtNroFactura.Text = Format(txtNroFactura.Text, "00000000")
        Else
            MsgBox "El Remito no existe o no corresponde con el Cliente", vbExclamation, TIT_MSGBOX
            txtNroFactura.Text = ""
