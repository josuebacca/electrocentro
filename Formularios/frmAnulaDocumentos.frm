...
Private Sub BuscoPedidos()
    ...
    sql = "SELECT NP.NPE_NUMERO, NP.NPE_FECHA, NP.EST_CODIGO, C.CLI_CODIGO, C.CLI_RAZSOC,E.EST_DESCRI"
    ...
    If rec.EOF = False Then
        Do While rec.EOF = False
            GrdModulos.AddItem Format(rec!NPE_NUMERO, "00000000") & Chr(9) & rec!NPE_FECHA _
                            & Chr(9) & Trim(rec!CLI_RAZSOC) & Chr(9) & Trim(rec!EST_DESCRI) _
...
Private Sub BuscoOrdenesCompra()
    ...
    If txtVendedor.Text <> "" Then sql = sql & " AND NP.VEN_CODIGO=" & XN(txtVendedor)
    If FechaDesde <> "" Then sql = sql & " AND NP.OC_FECHA>=" & XDQ(FechaDesde)
    If FechaHasta <> "" Then sql = sql & " AND NP.OC_FECHA<=" & XDQ(FechaHasta)
    ...
    If rec.EOF = False Then
        Do While rec.EOF = False
            GrdModulos.AddItem Format(rec!OC_NUMERO, "00000000") & Chr(9) & rec!OC_FECHA _
                            & Chr(9) & Trim(rec!PROV_RAZSOC) & Chr(9) & Trim(rec!EST_DESCRI) _
...
Private Sub BuscoFacturasProveedor()
    ...
    Screen.MousePointer = vbHourglass
    sql = "SELECT FC.FPR_NUMERO, FC.FPR_NROSUC, FC.FPR_FECHA,FC.EST_CODIGO, E.EST_DESCRI,"
...
Private Sub BuscoFacturas()
    lblestado.Caption = "Buscando Facturas..."
    Screen.MousePointer = vbHourglass
    sql = "SELECT FC.FCL_NUMERO, FC.FCL_SUCURSAL, FC.FCL_FECHA,FC.EST_CODIGO, E.EST_DESCRI,"
...
Private Sub BuscoGastosGrales()
    ...
    sql = "SELECT RC.GGR_NROCOMP , RC.GGR_NROSUC, RC.GGR_FECHACOMP,"
    ...
    If FechaDesde <> "" Then sql = sql & " AND RC.GGR_FECHACOMP>=" & XDQ(FechaDesde)
    If FechaHasta <> "" Then sql = sql & " AND RC.GGR_FECHACOMP<=" & XDQ(FechaHasta)
    ...
    If rec.EOF = False Then
        Do While rec.EOF = False
            GrdModulos.AddItem rec!TCO_ABREVIA & Chr(9) & Format(rec!GGR_NROSUC, "0000") & "-" & Format(rec!GGR_NROCOMP, "00000000") _
                               & Chr(9) & rec!GGR_FECHACOMP & Chr(9) & rec!PROV_RAZSOC _
                               & Chr(9) & rec!EST_DESCRI & Chr(9) & rec!EST_CODIGO _
                               & Chr(9) & rec!EST_CODIGO & Chr(9) & rec!TCO_CODIGO _
...
Private Sub BuscoOrdenPago()
    ...
    sql = "SELECT RC.OPG_NUMERO, RC.OPG_NROSUC, RC.OPG_FECHA,"
    sql = sql & " RC.TCO_CODIGO, TC.TCO_ABREVIA,RC.PROV_CODIGO,"
    sql = sql & " C.PROV_RAZSOC, E.EST_DESCRI, RC.EST_CODIGO,C.TPR_CODIGO"
    ...
    If txtCliente.Text <> "" Then sql = sql & " AND RC.PROV_CODIGO=" & XN(txtCliente)
    If txtVendedor.Text <> "" Then sql = sql & " AND RC.VEN_CODIGO=" & XN(txtVendedor)
    If FechaDesde <> "" Then sql = sql & " AND RC.OPG_FECHA>=" & XDQ(FechaDesde)
    If FechaHasta <> "" Then sql = sql & " AND RC.OPG_FECHA<=" & XDQ(FechaHasta)
    If chkTipo.Value = Checked Then sql = sql & " AND RC.TCO_CODIGO=" & XN(cboDocumento.ItemData(cboDocumento.ListIndex))
    ...
    If rec.EOF = False Then
        Do While rec.EOF = False
            GrdModulos.AddItem rec!TCO_ABREVIA & Chr(9) & Format(rec!OPG_NROSUC, "0000") & "-" & Format(rec!OPG_NUMERO, "00000000") _
                               & Chr(9) & rec!OPG_FECHA & Chr(9) & rec!PROV_RAZSOC _
                               & Chr(9) & rec!EST_DESCRI & Chr(9) & rec!EST_CODIGO _
                               & Chr(9) & rec!EST_CODIGO & Chr(9) & rec!TCO_CODIGO _
                               & Chr(9) & rec!PROV_CODIGO & Chr(9) & rec!TPR_CODIGO
    ...
    If rec.EOF = False Then
        Do While rec.EOF = False
            GrdModulos.AddItem rec!TCO_ABREVIA & Chr(9) & Format(rec!REC_SUCURSAL, "0000") & "-" & Format(rec!REC_NUMERO, "00000000") _
                               & Chr(9) & rec!REC_FECHA & Chr(9) & rec!CLI_RAZSOC _
                               & Chr(9) & rec!EST_DESCRI & Chr(9) & rec!EST_CODIGO _
                               & Chr(9) & rec!EST_CODIGO & Chr(9) & rec!TCO_CODIGO _
    ...
    For I = 1 To GrdModulos.Rows - 1
        If GrdModulos.TextMatrix(I, 5) <> GrdModulos.TextMatrix(I, 6) Then
            ...
            If Rec2.EOF = True Then
                ...
                sql = sql & " SET EST_CODIGO=1"
                sql = sql & " WHERE"
                sql = sql & " RCL_NUMERO=" & XN(Right(GrdModulos.TextMatrix(I, 9), 8))
                sql = sql & " AND RCL_SUCURSAL=" & XN(Left(GrdModulos.TextMatrix(I, 9), 4))
                sql = sql & " AND RCL_FECHA=" & XDQ(GrdModulos.TextMatrix(I, 10))
                DBConn.Execute sql
                
                'ACTUALIZO CTA-CTE
                DBConn.Execute QuitoCtaCteCliente(GrdModulos.TextMatrix(I, 8), GrdModulos.TextMatrix(I, 7), _
...
Private Sub ActualizoRecibo()
    ...
    For I = 1 To GrdModulos.Rows - 1
        If GrdModulos.TextMatrix(I, 5) <> GrdModulos.TextMatrix(I, 6) Then
            ...
            'ACTUALIZO EL SALDO DE LAS REM F
            sql = "SELECT FR.RCL_TCO_CODIGO, FR.RCL_NUMERO, FR.RCL_SUCURSAL, FR.RCL_FECHA,"
            sql = sql & " FR.REC_IMPORTE,FC.RCL_SALDO"
            sql = sql & " FROM REMITO_RECIBO_CLIENTE FR, REMITO_CLIENTE FC"
            sql = sql & " WHERE"
            sql = sql & " FR.TCO_CODIGO=" & XN(GrdModulos.TextMatrix(I, 7))
            ...
            sql = sql & " AND FR.RCL_TCO_CODIGO=FC.TCO_CODIGO"
            sql = sql & " AND FR.RCL_NUMERO=FC.RCL_NUMERO"
            sql = sql & " AND FR.RCL_SUCURSAL=FC.RCL_SUCURSAL"
            
            
            rec.Open sql, DBConn, adOpenStatic, adLockOptimistic
            If rec.EOF = False Then
                Do While rec.EOF = False
                    SaldoFactura = CDbl(rec!REC_IMPORTE) + CDbl(rec!RCL_SALDO)
                    sql = "UPDATE REMITO_CLIENTE"
                    sql = sql & " SET RCL_SALDO=" & XN(SaldoFactura)
                    sql = sql & " WHERE"
                    sql = sql & " TCO_CODIGO=" & XN(rec!RCL_TCO_CODIGO)
                    sql = sql & " AND RCL_NUMERO=" & XN(rec!RCL_NUMERO)
                    sql = sql & " AND RCL_SUCURSAL=" & XN(rec!RCL_SUCURSAL)
                    DBConn.Execute sql
                    SaldoFactura = "0"
                    rec.MoveNext
                Loop
...
Private Sub ActualizoFacturaProveedor()
    For I = 1 To GrdModulos.Rows - 1
        If GrdModulos.TextMatrix(I, 5) <> GrdModulos.TextMatrix(I, 6) Then
            ...
            sql = sql & " WHERE"
            sql = sql & " FPR_TCO_CODIGO=" & XN(GrdModulos.TextMatrix(I, 7))
            ...
            If Rec2.EOF = True Then
                ...
                sql = sql & " SET EST_CODIGO=1"
                sql = sql & " WHERE"
                sql = sql & " RPR_NUMERO=" & XN(Right(GrdModulos.TextMatrix(I, 9), 8))
                sql = sql & " AND RPR_SUCURSAL=" & XN(Left(GrdModulos.TextMatrix(I, 9), 4))
                sql = sql & " AND RPR_FECHA=" & XDQ(GrdModulos.TextMatrix(I, 10))
                DBConn.Execute sql
                
                'ACTUALIZO CTA-CTE
                DBConn.Execute QuitoCtaCteProveedores(GrdModulos.TextMatrix(I, 12), GrdModulos.TextMatrix(I, 8), _
...
Private Sub ActualizoOrdenPago()
    ...
    For I = 1 To GrdModulos.Rows - 1
        If GrdModulos.TextMatrix(I, 5) <> GrdModulos.TextMatrix(I, 6) Then
            ...
            sql = sql & " WHERE"
            sql = sql & " TCO_CODIGO=" & XN(GrdModulos.TextMatrix(I, 7))
            sql = sql & " AND OPG_NROCOMP=" & XN(Right(GrdModulos.TextMatrix(I, 1), 8))
...
Private Sub ConfiguroGrillaGastos()
    ...
    If TipodeAnulacion = 8 Then
        GrdModulos.FormatString = "^Tipo Gasto|^Número|^Fecha|Proveedor|^Estado|codigo estado|" _
    ...
    GrdModulos.ColWidth(9) = 0    'REPRESENTADA
    GrdModulos.Cols = 10
    GrdModulos.Rows = 2
...
Private Sub ConfiguroGrillaRecibo()
    ...
    If TipodeAnulacion = 8 Then
        GrdModulos.FormatString = "^Tipo Rec|^Número|^Fecha|Proveedor|^Estado|codigo estado|" _
                                & "codigo estado que cambio|TIPO RECIBO|COD CLIENTE|REPRESENTADA"
    Else
        GrdModulos.FormatString = "^Tipo Rec|^Número|^Fecha|Cliente|^Estado|codigo estado|" _
                                & "codigo estado que cambio|TIPO RECIBO|COD CLIENTE|REPRESENTADA"
    End If
    GrdModulos.ColWidth(0) = 1000 'TIPO_RECIBO
    GrdModulos.ColWidth(1) = 1300 'NRO RECIBO
...
Private Sub ConfiguroGrillaFactura()
    If TipodeAnulacion = 7 Then
        GrdModulos.FormatString = "^Tipo Fac|^Número|^Fecha|Proveedor|^Estado|codigo estado|" _
                                & "codigo estado QUE CAMBIO|TIPO FACTURA|COD CLIENTE|" _
                                & "NRO REMITO|FECHA REMITO|STOCK CODIGO|REPRESENTADA"
    Else
        GrdModulos.FormatString = "^Tipo Fac|^Número|^Fecha|Cliente|^Estado|codigo estado|" _
                                & "codigo estado QUE CAMBIO|TIPO FACTURA|COD CLIENTE|" _
                                & "NRO REMITO|FECHA REMITO|STOCK CODIGO|REPRESENTADA"
    End If
    ...
    GrdModulos.ColWidth(8) = 0    'CODIGO CLIENTE
    GrdModulos.ColWidth(9) = 0    'NUMERO REMITO
    GrdModulos.ColWidth(10) = 0   'FECHA REMITO
    GrdModulos.ColWidth(11) = 0   'STOCK CODIGO
...
Private Sub ConfiguroGrillaRemito()
    ...
    GrdModulos.ColWidth(1) = 1200 'FECHA
    GrdModulos.ColWidth(2) = 3850 'CLIENTE
    GrdModulos.ColWidth(3) = 2000 'ESTADO
    GrdModulos.ColWidth(4) = 0    'CODIGO ESTADO
    GrdModulos.ColWidth(5) = 0    'CODIGO ESTADO QUE CAMBIO
    GrdModulos.ColWidth(6) = 0    'CODIGO CLIENTE
    GrdModulos.ColWidth(7) = 0    'NUMERO Presupuesto
    GrdModulos.ColWidth(8) = 0    'FECHA Presupuesto
    GrdModulos.ColWidth(9) = 0    'REMITO SIN FACTURA
    GrdModulos.ColWidth(10) = 0   'CODIGO STOCK
    GrdModulos.Cols = 11
    GrdModulos.Rows = 2
End Sub
        ...
        Select Case TipodeAnulacion
            Case 1 'Presupuestos
                If GrdModulos.TextMatrix(GrdModulos.RowSel, 4) = 2 Then
                    ...
                    Exit Sub
                End If
                If GrdModulos.TextMatrix(GrdModulos.RowSel, 4) = 3 Then
                    MsgBox "No se puede cambiar el estado al Presupuesto" & Chr(13) & _
                           " ya que esta asignado a un Remito", vbExclamation, TIT_MSGBOX
                    
                    Exit Sub
                End If
                If GrdModulos.TextMatrix(GrdModulos.RowSel, 5) = 1 Then
                ...
                ElseIf GrdModulos.TextMatrix(GrdModulos.RowSel, 5) = 2 Then
                    GrdModulos.TextMatrix(GrdModulos.RowSel, 5) = 1
                    GrdModulos.TextMatrix(GrdModulos.RowSel, 3) = "PENDIENTE"
            ...
            Case 2 'REMITOS
                If GrdModulos.TextMatrix(GrdModulos.RowSel, 4) = 2 Then
                    ...
                    Exit Sub
                End If
                If GrdModulos.TextMatrix(GrdModulos.RowSel, 4) = 3 And GrdModulos.TextMatrix(GrdModulos.RowSel, 9) = "N" Then
                    MsgBox "No se puede cambiar el estado al Remito" & Chr(13) & _
                ...
                ElseIf GrdModulos.TextMatrix(GrdModulos.RowSel, 5) = 3 And GrdModulos.TextMatrix(GrdModulos.RowSel, 9) = "S" Then
                    GrdModulos.TextMatrix(GrdModulos.RowSel, 5) = 2
                    GrdModulos.TextMatrix(GrdModulos.RowSel, 3) = "ANULADO"
                    Call CambiaColorAFilaDeGrilla(GrdModulos, GrdModulos.RowSel, vbRed)
                End If
            ...
            Case 5 'orden compa
                If GrdModulos.TextMatrix(GrdModulos.RowSel, 4) = 2 Then
                    ...
                    Exit Sub
                End If
                If GrdModulos.TextMatrix(GrdModulos.RowSel, 4) = 4 Then
                    MsgBox "No se puede cambiar el estado de la Orden de Compra" & Chr(13) & _
