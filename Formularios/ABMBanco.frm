    ...
    If Trim(TxtBanCodigo.Text) <> "" Then
        ...
        sql = " SELECT CH.CHE_NUMERO " & _
              ...
              " WHERE B.BAN_CODINT = CH.BAN_CODINT " & _
                 "AND B.BAN_CODINT = " & XS(TxtCodInt.Text)
...
Private Sub CmdBuscAprox_Click()
    ...
    Me.Refresh
    sql = "SELECT BAN_DESCRI,BAN_NOMCOR,BAN_CODINT " & _
...
Private Sub CmdGrabar_Click()
    ...
    If Trim(TxtBanDescri.Text) = "" Or _
       Trim(TxtBanNomCor.Text) = "" Then
...
'            Exit Sub
'       Else
        If Trim(TxtBanDescri.Text) = "" Then
    ...
    sql = "SELECT BAN_DESCRI FROM BANCO " & _
          "WHERE BAN_CODINT = " & XN(TxtCodInt.Text)
    ...
    Else
        ...
        Rec2.Close
        sql = "INSERT INTO BANCO (BAN_CODINT," & _
                       "BAN_DESCRI,BAN_NOMCOR) VALUES " & _
                       "(" & XN(TxtCodInt) & "," & _
                       XS(TxtBanDescri.Text) & _
...
Private Sub TxtCodInt_LostFocus()
     Set rec = New ADODB.Recordset
    If TxtCodInt.Text <> "" Then
        sql = "SELECT BAN_DESCRI, BAN_NOMCOR FROM BANCO"
        sql = sql & " WHERE BAN_CODINT = " & XN(TxtCodInt)
        rec.Open sql, DBConn, adOpenStatic, adLockOptimistic
        If rec.EOF = False Then
            TxtBanDescri.Text = rec!BAN_DESCRI
            TxtBanNomCor.Text = rec!BAN_NOMCOR
        Else
            MsgBox "Codigo de Banco Inexistente", vbExclamation, TIT_MSGBOX
            TxtBanDescri.Text = ""
            TxtBanNomCor.Text = ""
            TxtCodInt.SetFocus
        End If
